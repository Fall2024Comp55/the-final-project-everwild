...
public class BattlePane extends GraphicsPane {
	...
	private void addSecondDescription(boolean hasWon) {
		...
		if (hasWon)
			descriptionLabel2
					.setLabel("Your monster can keep fighting this turn! (click anywhere to return to training)");
		else
			descriptionLabel2.setLabel("Your monster can't continue this turn! " + mainScreen.getTurnsRemaining()
					+ " turns remaining. (click anywhere to return to training)");
				...
				if (playerImg.getBounds().intersects(enemyImg.getBounds())) {
					...
					for (int i = 0; i < enemyOverPlayerAgility; ++i) {
						...
						if (playerDodgeCheck > Math.min(20, playerMonster.getSpeed())) {
							...
							if (enemyCritCheck <= enemyMonster.getSpeed()
									&& mainScreen.getDifficulty() != Difficulty.BABY) {
								enemyDamage *= 2;
	...
	private void finishBattle() {
		if (battleDifficulty == BattleDifficulty.BOSS) {
			...
			if (playerTempHealth <= 0 && enemyTempHealth <= 0) {
				...
				updateDescription("Both monsters fainted! It's a draw! Gained " + (difficultyModifier + 1)
						+ " fatigue. Your monster seems determined after that tie.");
				...
				mainScreen.setTurnsRemaining(mainScreen.getTurnsRemaining() - 1);
				updateDescription("Your monster fainted... Gained " + (difficultyModifier * 2)
						+ " fatigue. Your monster seems tougher after that beatdown.");
				addSecondDescription(false);
				playerMonster.setFatigue(playerMonster.getFatigue() + difficultyModifier * 2);
				playerMonster.setHealth(playerMonster.getHealth() + difficultyModifier + 1);
			} else {
				updateDescription("The enemy monster fainted! You won the battle! Gained " + (difficultyModifier + 1)
						+ " fatigue. Your monster seems stronger now.");
